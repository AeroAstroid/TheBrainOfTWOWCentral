...
async def on_ready():
    ...
    async def on_message(message):
        ...
        try:
            ...
            if message.author.id in [184768535107469314, 183331874670641152, 179686717534502913]:
                perms = 3
            elif message.author in msg_guild["STAFF_ROLE"].members:
                perms = 2
            elif message.author in msg_guild["MEMBER_ROLE"].members:
                perms = 1
            else:
                perms = 0
            ...
            if state is not None:
                if state[0] == 0:  # The restart command returns a [0] flag, signaling that the bot should be restarted
                    ...
                    os.execl(
                        sys.executable, 'python', __file__, r_guild, str(message.channel.id), str(time.time()),
                        *extra_args)
