...
from src.interpreter.functions.raise_func import BStarProgramDefinedException
...
def runCodeSandbox(code: Tree, user: Union[discord.User, None] = None, arguments: List[str] = []):
    ...
    if len(globals.codebase.output) == 0 or globals.codebase.output.isspace():
        return discord.Embed(description="⚠️: The code has ran successfully, but returned nothing!")
    if len(globals.codebase.output) > 4000:
        return discord.Embed(description=f"⚠️: Output too long, only showing the first 3800 characters:\n\n```{globals.codebase.output[:3800]}```")
    return discord.Embed(description=globals.codebase.output)
...
def returnError(statement, error, i):
    ...
    return discord.Embed(description=errmsg)
