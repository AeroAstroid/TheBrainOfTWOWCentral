from Helper.__comp import *
import inspect
from io import StringIO as sIO

import re

from Helper.__functions import is_dev, plural, m_line, is_number, is_whole, split_escape, clock_scheduler

def setup(BOT):
	BOT.add_cog(MemeChannelRenamer(BOT))

class MemeChannelRenamer(cmd.Cog):
	'''
	Task that changes the name of TWOW Central's #memes channel every hour
	'''

	###### BEGIN CUSTOM TASK PARAMETERS! ######

	CHANNEL_ID = 656641155869704205
	LETTERS = "éêèëĕẽēė"
	CHANNEL_NAME = "mem(.)s"

	####### END CUSTOM TASK PARAMETERS! #######

	def __init__(self, BRAIN):
		self.BRAIN = BRAIN

		for c in self.__cog_commands__:
			c.name = self.TASK_PREFIX + c.name
	
	TASK_PREFIX = "mcr/" # Secondary prefix for this this task's commands
	ON_BY_DEFAULT = True # If true, will get auto-started once the bot fully connects

	############ BEGIN TASK CODE! ############

	@tasks.loop(time=clock_scheduler(3600))
	async def change_channel_name(self):
		channel = self.BRAIN.get_channel(self.CHANNEL_ID)

		if not (m := re.match(self.CHANNEL_NAME, channel.name)):
			new_name = self.CHANNEL_NAME.replace("(.)", self.LETTERS[0])
		
		elif (i := self.LETTERS.find(m.groups()[0])) < 0:
			new_name = self.CHANNEL_NAME.replace("(.)", self.LETTERS[0])
		
		else:
			new_name = self.CHANNEL_NAME.replace("(.)", self.LETTERS[(i+1) % len(self.LETTERS)])
		
		await channel.edit(name=new_name)

		self_loop = self.get_a_loop("change_channel_name")[1]
		if self_loop.seconds is None:
			self_loop.change_interval(seconds=3600)
		

	############# END TASK CODE! #############

	# Below is all boilerplate task code -- preferrably do not edit it at all
	
	def get_parameters(self):
		'''
		Returns a list of all task-specific parameters and values
		'''

		task_attrs = [m for m in inspect.getmembers(self)]
		cog_attrs = [m[0] for m in inspect.getmembers(cmd.Cog)]

		# Hardcoded here are default parameters that the command user shouldn't have access to
		attrs = [m for m in task_attrs if m[0] not in (cog_attrs + 
			["TASK_PREFIX", "BRAIN", "ON_BY_DEFAULT"]
		)]

		# Exclude functions, loops and commands from the attributes list
		attrs = [m for m in attrs if (
			not inspect.ismethod(m[1]) and
			not type(m[1]) == tasks.Loop and
			not type(m[1]) == cmd.core.Command
		)]

		return attrs
	
	@cmd.command()
	@cmd.check(is_dev)
	async def params(self, ctx):
		'''
		Command to display all task-specific parameters and values
		'''

		params = self.get_parameters()

		param_msg = sIO()
		param_msg.write(f"**Editable parameters in the `{self.qualified_name}` task:**\n\n")

		for name, value in params:
			param_msg.write(f"> **{name}** ({type(value)})\n")
			param_msg.write(f"> `{value}`\n\n")

		await ctx.reply(param_msg.getvalue())
		return

	@cmd.command()
	@cmd.check(is_dev)
	async def edit(self, ctx, parameter, *value):
		'''
		Command to edit a parameter's value
		'''

		attrs = self.get_parameters()

		param_list = [p[0] for p in attrs]

		if parameter.upper() not in param_list:
			await ctx.reply(m_line(f"""💀 **Invalid parameter `{parameter.upper()}`!** 
			Available parameters are: /n/> **`{", ".join(param_list)}`**"""))
			return
		
		i = param_list.index(parameter.upper())
		value = " ".join(value)

		try: # Prioritize keeping the same type as the original variable
			value = type(attrs[i][1])(value)

		except Exception: # If that's not possible...
			if is_whole(value):
				value = int(value)
			elif is_number(value):
				value = float(value)
			elif len(split_escape(value, ", ")) > 1:
				value = split_escape(value)
			
			# If all of these fail, keep it a string

		setattr(self, parameter.upper(), value)

		await ctx.reply(m_line(f"""
		**Successfully edited parameter `{parameter.upper()}`** to the value: /n/> **`{value}`**
		"""))
		return
	
	def get_loops(self):
		'''
		Returns all loops from the task
		'''

		return [m for m in inspect.getmembers(self) if type(m[1]) == tasks.Loop]
	
	def get_a_loop(self, name):
		'''
		Returns a specific named loop from the task, or False if it doesn't exist
		'''

		matching_loops = [m for m in self.get_loops() if m[0].lower() == name.lower()]

		if matching_loops:
			return matching_loops[0]
		else:
			return False
	
	@cmd.command()
	@cmd.check(is_dev)
	async def status(self, ctx):
		'''
		Command to check the status of every loop in the task
		'''

		loop_msg = sIO()
		loop_msg.write(f"**Status for loops in the `{self.qualified_name}` task:**\n\n")

		for l, obj in self.get_loops():
			loop_msg.write(
			f"> **{l.upper()}** - {'Running' if obj.is_running() else 'Not Running'}\n")

			if obj.is_running():
				loop_msg.write(m_line(f"""
				> Next iteration: **<t:{obj.next_iteration.timestamp():.0f}:f>** 
				(**<t:{obj.next_iteration.timestamp():.0f}:R>**)/n//n/"""
				))
		
		await ctx.reply(loop_msg.getvalue())
		return
	
	def set_loop(self, start, loop_name=''):
		'''
		Sets the status of one (or all) of the loops to either running or not running
		'''

		if not loop_name:
			all_loops = self.get_loops()

			for loop_tuple in all_loops:
				if start:
					loop_tuple[1].start()
				else:
					loop_tuple[1].cancel()
			
			return all_loops
		
		found_loop_tuple = self.get_a_loop(loop_name)

		if not found_loop_tuple:
			return False

		if start:
			found_loop_tuple[1].start()
		else:
			found_loop_tuple[1].cancel()
		
		return found_loop_tuple

	@cmd.command()
	@cmd.check(is_dev)
	async def start(self, ctx, loop_name=''):
		'''
		Command to start one (or all) loop(s)
		'''

		try:
			result = self.set_loop(True, loop_name=loop_name)
		except RuntimeError:
			pass

		if not result:
			await ctx.reply(f"💀 **Loop `{loop_name}` wasn't found in `{self.qualified_name}`!**")
			return
		
		if type(result) == list:
			await ctx.reply(f"**Successfully started {len(result)} loop{plural(len(result))}!**")
			return

		await ctx.reply(f"**Successfully started loop `{result[0]}`!**")
		return

	@cmd.command()
	@cmd.check(is_dev)
	async def end(self, ctx, loop_name=''):
		'''
		Command to end one (or all) loop(s)
		'''

		try:
			result = self.set_loop(False, loop_name=loop_name)
		except RuntimeError:
			pass

		if not result:
			await ctx.reply(f"💀 **Loop `{loop_name}` wasn't found in `{self.qualified_name}`!**")
			return
		
		if type(result) == list:
			await ctx.reply(f"**Successfully ended {len(result)} loop{plural(len(result))}!**")
			return

		await ctx.reply(f"**Successfully ended loop `{result[0]}`!**")
		return