...
def USERNAME() -> tuple: return ("n", )

def USERID() -> tuple: return ("id", )
...
def ARGS(idx: int | None = None) -> tuple:
...
def GLOBALDEFINE(gvar: str, value: object) -> tuple:
	if type(gvar) != str:
	...
	return ("gd", (gvar, value))

def GLOBALVAR(gvar: str) -> tuple:
	if type(gvar) != str:
		raise NameError(f"Global variable name must be a string: {safe_cut(gvar)}")
...
def DEFINE(var_name: str, value: object) -> tuple:
	if type(var_name) != str:
		raise NameError(f"Variable name must be a string: {safe_cut(var_name)}")
	
	return ("d", (var_name, value))

def VAR(var_name: str) -> tuple:
	if type(var_name) != str:
		raise NameError(f"Variable name must be a string: {safe_cut(var_name)}")
