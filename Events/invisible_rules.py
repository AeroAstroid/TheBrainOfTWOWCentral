...
class EVENT:
	...
	def make_timer(self, remaining, just_timestamp=False):
		round_t = self.GAME["ROUND_TIME"][self.GAME["ROUND"]]
	...
	async def on_two_second(self):
		...
		if self.GAME["ROUND"] > 0: # Ending a round
			...
			if self.GAME["PERIOD_STEP"] == 7:
				results_list = [
					...
					[p, self.GAME["TOTAL_POINTS"][self.GAME["ALL_PLAYERS"].index(p)], 0, 0, 9999999, False]
			...
			if self.GAME["PERIOD_STEP"] == 19:
				...
				if self.GAME["ROUND"] >= len(self.GAME["RULES"]) or len(self.GAME["PLAYERS"]) <= 1:
					await self.ANNOUNCE_CHANNEL.send("Invisible Rules has finished!")
					final_results = []

					for p, score in zip(self.GAME["ALL_PLAYERS"], self.GAME["TOTAL_POINTS"]):
				...
				if self.GAME["ROUND"] != self.PARAM["PHASE_1_LEN"]:
					...
					round_str = f"This round is worth **{self.GAME['MAX_POINTS'][new_round]}** points and it will last"
					m, s = self.GAME["ROUND_TIME"][new_round]//60, self.GAME["ROUND_TIME"][new_round]%60
					if m > 0:
						round_str += " **1 minute**" if m == 1 else f" **{m} minutes**"
					if s > 0:
						round_str += " **1 second**" if s == 1 else f" **{s} seconds**"
					round_str += "."
					await self.ANNOUNCE_CHANNEL.send(round_str)
		...
		if self.GAME["ROUND"] == 0: # Intermission between phases
			...
			if self.GAME["PHASE"] == 1:
				...
				decay_str = f"{self.PARAM['MINUTE_PENALTY']*100:.2f}%"
				...
				lines = [
					...
					m_line(f"""> For this phase, points now depend on speed and accuracy."""),
				...
				if self.PARAM["PHASE_1_LEN"] == 0:
					lines = lines[3:]
					lines[0] = m_line(f"""> Once the round starts, everyone playing will be able to **INSPECT** the current 
					rule by sending messages in <#{self.GAME_CHANNEL.id}>. The bot will react to every message 
					that doesn't use invalid characters with a ✅ **if it passes the rule**, or a ❌ **if it 
					breaks the rule.** If a message **has invalid characters**, the bot will react with ❔ regardless 
					of whether it passes the rule or not. You may send as many messages as you want. There is no 
					penalty or reward for specifically sending messages that break or pass the rule. Valid characters 
					are letters and spaces. Rules are case-insensitive. All other characters are invalid.""")
					lines[1] = m_line(f"""> To start **TESTING**, DM me with **`ir/test`**. You will be 
					sequentially shown a series of {self.PARAM['PHASE_2_TEST_LEN']} messages, one by one, and must 
					answer whether or not they PASS or BREAK the current rule as they come.""")
					lines[3] = m_line(f"""> **You may go back to INSPECTING even after starting a 
					TEST** by DMing me with **`ir/inspect`**. You will be given access to <#{self.GAME_CHANNEL.id}> 
					again and will be allowed to read/send more messages. Once you go back to TESTING using 
					**`ir/test`** as usual, you will be given a completely new test.""")
			...
			if self.GAME["PERIOD_STEP"] % message_delay == 0:
				...
				if ind >= len(lines):
					...
					round_str = f"This round is worth **{self.GAME['MAX_POINTS'][-self.GAME['ROUND']]}** points and it will last"
					m, s = self.GAME["ROUND_TIME"][-self.GAME["ROUND"]]//60, self.GAME["ROUND_TIME"][-self.GAME["ROUND"]]%60
					if m > 0:
						round_str += " **1 minute**" if m == 1 else f" **{m} minutes**"
					if s > 0:
						round_str += " **1 second**" if s == 1 else f" **{s} seconds**"
					round_str += "."
					await self.ANNOUNCE_CHANNEL.send(round_str)
					if self.GAME["POINT_CUTOFFS"][-self.GAME["ROUND"]] > 0:
						await self.ANNOUNCE_CHANNEL.send(f"After this round, contestants below **{self.GAME['POINT_CUTOFFS'][-self.GAME['ROUND']]}** points will be eliminated.")
					return
		...
		if not self.GAME_STARTED:
			...
			if cmd == "setrules":
				...
				try:
					...
					POINT_CUTOFFS = TEMP_IR_RULES.POINT_CUTOFFS
					ROUND_TIME = TEMP_IR_RULES.ROUND_TIME
				...
				self.GAME["POINT_CUTOFFS"] = POINT_CUTOFFS
				self.GAME["ROUND_TIME"] = ROUND_TIME
		...
		if self.GAME["PHASE"] == 1:
			if n < self.PARAM["PHASE_1_TEST_LEN"]:
				...
				self.GAME["PLAYER_TESTS"][user_test_ind][5] = (
					self.GAME["ROUND_TIME"][self.GAME["ROUND"]] - (self.GAME["NEXT_PERIOD"] - time()))
